...
start:
	...
	ST		#3,3			 
	ST		#4,4
	STM		#205h,AR1 ;用间接寻址和RPT指令，将绝对地址205H开始的4个单元存成1234H	
	RPT		#3	; 重复执行下条语句4次
	ST		#1234h,*AR1+		; 完成数据存储
...
loop:
	...
	STM 	#2011H,AR0				
	RPT     #7              ; 设置循环次数为 7，共复制 8 个数据（包括首次复制）
	MVPD    table, *AR0+   	; MVPD 指令
							; 将表格 table 的数据传送到 *AR0+ 指向的地址
							; AR0 是一个地址寄存器，*AR0+ 表示以 AR0 当前指向的地址为起点，自动增加的寻址方式
							; 该指令常用于数据批量传输，表格 table 必须在指令执行前已经定义并载入所需数据
							;  参数 table - 需要传送的数据表格  *AR0+ - 数据接收地址，自动增加的方式读取地址
xh:
	B 		xh			; 空循环
	.end
